...
from app.domain.models.campaign import Party, Character, Encounter, Quest, JournalEntry
...
class CharacterRepository(SqliteRepository):
    ...
    def list_by_campaign(self, campaign_id: str) -> list[Character]:
        rows = self.conn.execute(
...
class QuestRepository(SqliteRepository):
    ...
    def list_by_party(self, party_id: str) -> list[Quest]:
        rows = self.conn.execute(
            "SELECT * FROM quests WHERE party_id = ?", (party_id,)
        ).fetchall()
        return [self._record_to_instance(dict(row)) for row in rows]

    def list_active(self, party_id: str) -> list[Quest]:
        rows = self.conn.execute(
            "SELECT * FROM quests WHERE party_id = ? AND status IN ('available', 'active')",
            (party_id,),
        ).fetchall()
        return [self._record_to_instance(dict(row)) for row in rows]
